...
* replace the use of eval with string manipulation
  for uninstall_key list server side;

* cosmetic: limit repo row height when description is long;

* Add option for *use_fqdn_as_uuid* when building :program:`waptagent.exe`;

* waptwua options in :file:`ini` file with its own section ``[waptwua]``
  (**Enterprise**);

* handle wua offline / online scan param in :file:`wapt-get.ini`
  (**Enterprise**);

* store Windows Update rules in local db as a dict (**Enterprise**);

* use *_* instead of *-* in update_id to separate *UpdateId*
  with *RevisionNumber* (**Enterprise**)
...
* fix *package_uuid* not sent to server's host inventory:
  ...
  * creates a random *package_uuid* when installing a package
    without *package_uuid*;
...
* display a red icon in host package list when a package must be removed
  due to conflicts or change of Organizational Unit;

* fix :command:`wapt-get sign-packages` key password dialog:

  * added setting *maturity* and increment automatically the version
    in :command:`sign-packages` action  in :command:`sign-packages` action;
...
* Add WindowsUpdates host history grid below :guilabel:`WindowsUpdate` grid
  (**Enterprise**):
  ...
  * refresh in packages and groups with :guilabel:`Show Hosts` active,
    refresh :guilabel:`Hosts` list too;
